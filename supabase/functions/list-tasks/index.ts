...
serve(async (req) => {
  ...
  try {
    const { page = 1, limit = 20, type, userId, apiKey: providedApiKey } = await req.json();
    ...
    console.log(`Listing tasks: page=${page}, limit=${limit}, type=${type || 'all'}`);

    const response = await fetch(`https://api.ai33.pro/v1/tasks?${params.toString()}`, {
      method: "GET",
      headers: {
        "xi-api-key": API_KEY,
    ...
    console.log(`Listed ${data.data?.length || 0} tasks`);
